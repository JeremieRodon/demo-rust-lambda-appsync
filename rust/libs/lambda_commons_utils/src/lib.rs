...
macro_rules! lambda_main {
    (common $sync_or_async:ident $rc:ident($event_type:ty)->$return_type:ty $(, $code:stmt)?) => {
        ...
        async fn function_handler(
            event: $crate::lambda_runtime::LambdaEvent<$crate::serde_json::Value>,
        ) -> Result<$return_type, $crate::lambda_runtime::Error> {
            $crate::log::debug!("{event:?}");
            $crate::log::info!("{}", $crate::serde_json::json!(event.payload));
            let payload: $event_type = $crate::serde_json::from_value(event.payload)?;
