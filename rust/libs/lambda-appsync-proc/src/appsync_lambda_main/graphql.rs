...
impl Operations {
    fn variants_iter(&self) -> impl Iterator<Item = proc_macro2::Ident> {
...
impl TryFrom<(Document<'_, String>, proc_macro2::Span)> for GraphQLSchema {
    type Error = syn::Error;

    fn try_from(
        (mut doc, span): (Document<'_, String>, proc_macro2::Span),
                    ...
                    match type_definition {
                        TypeDefinition::Object(object_type) => {
                            if let Some(sdt) = sd.schema_definition(&object_type.name) {
                                ...
                                structures.push(Structure::from(object_type));
                            }
                        }
                        ...
                        TypeDefinition::InputObject(input_object_type) => {
                            structures.push(Structure::from(input_object_type));
                        }
            ...
            impl Operation {
                pub async fn execute(self, args: ::lambda_appsync::serde_json::Value) -> ::lambda_appsync::AppSyncResponse {
                    match self._execute(args).await {
                        ::core::result::Result::Ok(v) => ::lambda_appsync::AppSyncResponse {
                            data: ::core::option::Option::Some(v),
...
impl ToTokens for GraphQLSchema {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
